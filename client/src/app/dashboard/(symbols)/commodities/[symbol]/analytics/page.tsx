...
export default function CommodityAnalyticsPage() {
  ...
  const handleStartAnalysis = async () => {
    if (!commodity || (!analysisTypes.news && !analysisTypes.chart)) return;
            ...
            <div className="text-center">
                  ...
                  <div className={`flex items-center gap-2 justify-center ${priceChangeDisplay.color}`}>
                    ...
                    <span className="text-lg font-semibold">
                      {changePercentage !== undefined ? (changePercentage >= 0 ? '+' : '') + changePercentage.toFixed(2) + '%' : 'N/A'}
                    </span>
              </div>
                </div>
      ...
      <Card className="p-8 bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-950/20 dark:to-teal-950/20 border-0 shadow-xl">
        <CardHeader className="text-center pb-6">
          ...
          <p className="text-muted-foreground text-lg">
            {showAnalysisForm 
              ? `Get intelligent insights from multiple AI models for ${commodityName} analysis using ${selectedInterval} chart data`
              : `AI analysis completed for ${commodityName} using ${selectedInterval} data`
            }
          </p>
          </CardHeader>
        ...
        <Card className="p-6">
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <div className="text-sm text-muted-foreground">Display Symbol</div>
                <div className="font-semibold">{commodity.displaySymbol}</div>
              </div>
              <div>
